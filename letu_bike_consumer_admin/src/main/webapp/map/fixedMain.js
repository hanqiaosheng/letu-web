...
//      str.push('是否经过偏移：' + (data.isConverted ? '是' : '否'));
    ...
    function onError(data) {
    	/*$.alert("定位失败,刷新重新获取",true,function(){
...
function getTips(data){//后台搜索周围的车
		...
		markers = [];
      	$.ajax({
          url :"/api/fixedReturn/findFixeds.action",
          data:"latLng="+data,
          async : true,  
          type : "post", 
          beforeSend : function() {
...
//		      clearInterval(timer);
      ...
      function getFixedInfo(id){
    	  ...
    	  $.ajax({
    		  type:'post',
    		  url:'/api/fixedReturn/fixedReturnInfos.action',
    		  data:'fixedReturnId='+id,
    		  success : function(data) {
    			  if(data!=null&&data!=""){
    				  if(data.data.fixedReturn.fixedReturnBikeNum<0){
    					  $("#bikeNum").html(0);
    				  }else{
