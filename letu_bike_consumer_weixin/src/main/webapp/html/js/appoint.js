...
$("#cancleBtn").click(function(){
	...
	var dataStr ="bikeId="+bikeId+"&appointId="+appointId+"&userId="+userId;
	 $.ajax({
		 	type:"post",
		 	url:url+'/appoint/appointCancel.action',
		 	data:dataStr,
			dataType:'json',
			success:function(data){
				cancel();
...
function appoint(){
	...
	var dataStr ="bikeId="+bikeId+"&appointmentTime="+appointTime+"&appointmentPlanId="+appointmentPlanId;
	 $.ajax({
		 	type:"post",
			url:url+'/appoint/appointing.action',
			data:dataStr,
			dataType:'json',
			success:function(data){
				$.isLoading.hide();
