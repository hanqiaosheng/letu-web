...
public class MonitorRealm extends AuthorizingRealm {
	...
	@Resource
	RoleServiceReport roleServiceReport;
	
	@Resource
	PermissionServiceReport permissionServiceReport;
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
        ...
        try {
			...
			if(admin!=null){
				//查当前拥有的角色
				List<AdminToRoleKey> rolecontact = roleServiceReport.findKeyByAId(admin.getAdminId());
				if(rolecontact!=null && rolecontact.size()>0){
					for (AdminToRoleKey ro : rolecontact) {
						Role role = roleServiceReport.findById(ro.getRoleId());;
						if(null!=role&&role.getRoleState()!=-1&&role.getRoleState()!=0){
							roles.add(role.getRoleName());
							List<RoleToPermissionKey> rpList = permissionServiceReport.findByRId(role.getRoleId());
							if(rpList!=null && rpList.size()>0){
								for (RoleToPermissionKey rp : rpList) {
									Permission permission = permissionServiceReport.findByPId(rp.getPermissionId());
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		Admin admin = null;
		try {
			admin = adminServiceReport.findByName(token.getUsername());
